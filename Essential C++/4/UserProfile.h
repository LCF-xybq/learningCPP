...
public:
  ...
  int guess_count() const { return _guessed; }
  int guess_correct() const { return _correct_guessed; }
  ...
  void reset_guess_count( int val ) { _guessed = val; }
  void reset_guess_correct( int val ) { _correct_guessed = val; }

  void bump_login_count( int cnt = 1 ) { _times_logged += cnt; }
  void bump_guess_count( int cnt = 1 ) { _guessed += cnt; }
  void bump_guess_correct( int cnt = 1 ) { _correct_guessed += cnt; }
...
inline double UserProfile::guess_average() const {
  return _guessed
        ? double(_correct_guessed) / double(_guessed) * 100
        : 0.0;
}

inline UserProfile::UserProfile( string login, uLevel level )
  : _login( login ), _user_level( level ),
    _times_logged( 1 ), _guessed( 0 ), _correct_guessed( 0 ){}
